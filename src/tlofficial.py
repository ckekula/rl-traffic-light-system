...
class Traffic_Light:
    ...
    def __init__(self, env, tl_id, delta_time, yellow_time, min_green, max_green):
        ...
        self.phases = traci.trafficlight.getCompleteRedYellowGreenDefinition(self.id)[
            0
        ].phases
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.ones(self.num_green_phases + len(self.lanes)),
            dtype=np.float32,
        )
    ...
    def set_next_phase(self, new_phase):
        ...
        if (
            ...
            or self.time_since_last_phase_change < self.yellow_time
        ):
            ...
            self.next_action_time = (
                self.env.sim_step + self.delta_time + self.yellow_time
            )
            self.is_yellow = True
            self.time_since_last_phase_change = 0
    ...
    def compute_reward(self):
        self.last_reward = (
            ...
            + self.fuel_reward()
            + self.passed_cars_reward()
            + self.noiseEmission_reward()
        )
    ...
    def get_out_lanes_density(self):
        ...
        return [
            min(
                ...
                / (traci.lane.getLength(lane) / vehicle_size_min_gap),
            )
    ...
    def get_total_queued(self):
        """
        Returns the total number of queued vehicles across all lanes
        """
